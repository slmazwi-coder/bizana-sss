    ...
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-10">
          <h2 className="text-3xl font-bold text-[#7B1B2B] flex items-center gap-2">
            <Bell className="text-yellow-500" /> News & Notices
          </h2>
          <button className="text-[#7B1B2B] font-semibold hover:underline">View All</button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {notices.map((notice) => (
              <div key={notice.id} className="card flex flex-col h-full border-l-4 border-l-[#D4AF37]">
                ...
                <button className="text-[#7B1B2B] font-medium flex items-center gap-1 hover:gap-2 transition-all">

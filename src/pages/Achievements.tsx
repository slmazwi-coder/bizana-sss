...
const StudentAvatar = ({ image, name, year }: { image: string; name: string; year: string }) => {
  ...
  return (
    <div className="aspect-[3/4] sm:aspect-square w-full relative overflow-hidden bg-gray-100 rounded-2xl shadow-lg border border-gray-100 flex items-center justify-center group">
      ...
      <div className="absolute top-0 right-0 bg-[#FDF9EC] p-4 text-[#7B1B2B] opacity-0 group-hover:opacity-100 transition-opacity">
        <Award size={24} />
      </div>
    </div>
  );
...
export const Achievements = () => {
  const [activeResultsYear, setActiveResultsYear] = useState<'2025'>('2025');
    ...
    <div className="py-12 sm:py-16 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="mb-16 sm:mb-24">
          <div className="bg-[#7B1B2B] border-2 border-[#D4AF37] rounded-3xl p-6 sm:p-8 md:p-12 relative overflow-hidden shadow-xl">
            <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none">
              <Star size={200} className="text-[#7B1B2B]" />
            </div>
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-10">
              <div className="w-32 h-32 sm:w-40 sm:h-40 bg-[#F5E6C8] rounded-full flex flex-col items-center justify-center text-[#7B1B2B] border-8 border-white shadow-lg shrink-0">
                <span className="text-2xl sm:text-3xl font-black">93.5%</span>
              ...
              <div>
                ...
                <p className="text-base sm:text-lg text-gray-700 max-w-2xl italic leading-relaxed text-center md:text-left">
                  "Congratulations to the Class of 2025! 277 learners wrote, achieving a 93.5% pass rate, with 179 Bachelor passes, 60 Diploma passes and 20 Higher Certificate passes."
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mb-20 sm:mb-32">
          <div className="text-center mb-10 sm:mb-12">
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[#7B1B2B] mb-4 flex items-center justify-center gap-4">
            ...
            {hallOfFame.slice(0, 8).map((student, idx) => (
              ...
              >
                ...
                <div className="p-6 text-center">
                  ...
                  <div className="text-[#7B1B2B] text-sm font-bold mb-3 flex items-center justify-center gap-1">
        ...
        <section className="mb-20 sm:mb-32">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 sm:mb-10">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#7B1B2B] flex items-center gap-3">
              <BarChart3 className="text-[#7B1B2B]" /> Matric Results Summary
            </h2>
            <div className="flex flex-wrap gap-2 bg-gray-100 p-1 rounded-xl">
              {(['2025'] as const).map((year) => (
                <button
                  key={year}
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                        ...
                        <div className="bg-white/10 p-5 sm:p-6 rounded-2xl backdrop-blur-sm border border-white/10 text-center md:text-left">
                          ...
                          <p className="text-white/80 text-sm font-medium">Total Passes</p>
                        </div>
                        <div className="bg-white/10 p-5 sm:p-6 rounded-2xl backdrop-blur-sm border border-white/10 text-center md:text-left">
                      ...
                      {currentResults.subjects.map((stat, i) => (
                        <div key={i} className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
                          <div className="flex justify-between items-center mb-3 gap-3">
                            ...
                            <span className="text-[#7B1B2B] font-bold">{stat.rate}%</span>
                          </div>
        ...
        <section>
          <div className="text-center mb-10 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl font-bold text-[#7B1B2B] flex items-center justify-center gap-3 mb-4">
              <Calendar className="text-[#7B1B2B]" /> Top Achievers Timeline
            </h2>
              ...
              <button
                ...
                className={`px-4 sm:px-5 py-2 rounded-full font-bold transition-all text-sm ${
                  activeAchieversYear === year
                    ...
                    : 'bg-white text-gray-500 border border-gray-200 hover:border-[#7B1B2B] hover:text-[#7B1B2B]'
                }`}
              >
                {year}
              </button>
            ...
            >
              {achieversByYear[activeAchieversYear] && achieversByYear[activeAchieversYear].length > 0 ? (
                achieversByYear[activeAchieversYear].map((person, i) => (
                  <div key={i} className="text-center w-[calc(50%-0.75rem)] sm:w-72">
                    <StudentAvatar image={person.image} name={person.name} year={activeAchieversYear} />
                    <div className="mt-4">
                      <h3 className="text-lg font-bold text-gray-900">{person.name}</h3>
                      <p className="text-xs font-semibold text-[#7B1B2B] uppercase tracking-wider">{person.title}</p>
                    </div>
                  </div>
                ))
              ) : (
